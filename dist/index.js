...
var syncableTableSchema = z.enum([
  "agents",
  "conversations",
  "messages",
  "usage_events",
...
var enqueueUpsertPayloadSchema = z.object({
  ...
  row: z.record(z.string(), z.unknown()).refine((row) => JSON.stringify(row).length <= 2e5, {
    message: "row payload exceeds 200KB \u2014 reject rather than silently truncate"
  })
});
var enqueueDeletePayloadSchema = z.object({
  table: syncableTableSchema,
  recordId: z.string().min(1).max(200)
});
var chatMessagePayloadSchema = z.object({
...
export { SATELLITE_ACTIONS, SatelliteController, SyncService, backfill, chatMessagePayloadSchema, enqueueDelete, enqueueDeletePayloadSchema, enqueueUpsert, enqueueUpsertPayloadSchema, getSupabaseClient, initSupabaseFromKeychain, newUuid, nextMessageSeq, syncableTableSchema };
